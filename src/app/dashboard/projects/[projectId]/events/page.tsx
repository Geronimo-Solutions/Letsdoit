...
export default async function MembersPage({
  ...
  const events = await getEventsUseCase(user, projectId);
  const isProjectOwner = await isProjectOwnerUseCase(user, projectId);
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        >
          Events
          {isProjectOwner && <CreateEventButton />}
        </h2>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-8">
                {upcomingEvents.map((event) => (
                  <EventCard isAdmin={isProjectOwner} event={event} />
                ))}
              </div>
            </div>
