...
export default async function InfoPage({ params }: { params: { projectId: string } }) {
  const { projectId } = params

  const user = await getCurrentUser()

  const isAdminOrOwner = await isAdminOrOwnerOfProjectUseCase(user, parseInt(projectId))
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col-reverse md:flex-row gap-8">
        ...
        <div
          className={cn(cardStyles, "w-full sm:w-[300px] p-8 sm:flex-shrink-0 space-y-8")}
        >
          <Image
